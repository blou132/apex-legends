...
static int explicitly_disable_breakpoint(
    ...
    if (set_first_hw_break_slot(pid, address, 0U) == -1) {
        return -1;
    }
    if (get_hw_break_state(pid, &state, &length) == -1 ||
            length != expected_length ||
            state.debug_registers[0].address != address ||
            (state.debug_registers[0].control & UINT32_C(1)) != 0U) {
        errno = EIO;
        return -1;
    }

    *cached_control = state.debug_registers[0].control;
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    enum bounded_wait_result wait_result;
    uint32_t disabled_cached_control = 0;
    ...
    if (argc != 3) {
        fprintf(stderr, "usage: ptrace_hw_breakpoint_runtime <pid> <address>\n");
        return 2;
    }
    ...
    if (set_first_hw_break_slot(target_pid, breakpoint_address, 0U) == -1) {
        print_errno("HW_BREAK_ADDR_SET_RESULT");
        goto cleanup;
    }
    puts("HW_BREAK_ADDR_SET_RESULT=SUCCESS");
    if (get_hw_break_state(target_pid, &readback_hw_state, &readback_hw_length) == -1 ||
            readback_hw_length != initial_hw_length ||
            readback_hw_state.debug_registers[0].address != breakpoint_address) {
        puts("HW_BREAK_ADDR_READBACK_MATCH=NO");
        goto cleanup;
    }
    puts("HW_BREAK_ADDR_READBACK_MATCH=YES");
    ...
    printf("HW_BREAK_CTRL_READBACK=0x%08" PRIx32 "\n",
    ...
    printf("HW_BREAK_DISABLED_CACHED_CONTROL=0x%08" PRIx32 "\n",
           disabled_cached_control);
...
cleanup:
    ...
    if (result != 0 && attached) {
        ...
        if (target_stopped && breakpoint_armed) {
            if (explicitly_disable_breakpoint(
                    target_pid,
                    breakpoint_address,
                    initial_hw_length,
                    &disabled_cached_control) == 0) {
                breakpoint_armed = 0;
                breakpoint_disabled = 1;
                printf("HW_FAILURE_CLEANUP_DISABLE=SUCCESS cached=0x%08" PRIx32 "\n",
                       disabled_cached_control);
            } else {
